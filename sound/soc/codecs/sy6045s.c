...
static int sy6045s_regs_init(struct sy6045s_priv *priv)
{
	int ret;

	/* Initialize amplifier registers from known defaults.
	 * Reg 0x07 (MISC_CONTROL_1): set bit 0 to enable I2C control
	 * Reg 0x06 (NFS_CONFIG): clear bit 3 for proper filter mode */
	ret = regmap_write(priv->regmap, 0x07, 0x01);
	if (ret)
		return ret;

	ret = regmap_update_bits(priv->regmap, 0x06, 0x08, 0x00);
	if (ret)
		return ret;
