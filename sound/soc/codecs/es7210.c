...
static void es7210_tdm_init_codec(u8 mode)
{
        ...
        * Mute All ADC
        */
        pr_debug("enter es7210_multi_chips_update_bits  %s\n",
               __func__);
        for (cnt = 0; cnt < ADC_DEV_MAXNUM; cnt++) {
...
static int es7210_suspend(struct snd_soc_component *component)
{
        ...
        pr_debug("jsl es7210_suspend enter\n");
...
static ssize_t es7210_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        ...
        pr_debug("eg read star address=0x06,count 0x10:echo 0610 >es7210\n");
        pr_debug("eg write star address=0x90,value=0x3c,count=4:echo 4903c >es7210\n");
        return 0;
}
...
static int es7210_i2c_probe(struct i2c_client *i2c_client)
{
	...
	unsigned int idx = 0;
        ...
        of_id = of_match_device(es7210_dt_ids, &i2c_client->dev);
        if (of_id)
                idx = (unsigned long)of_id->data;

        if (idx < ADC_DEV_MAXNUM) {
                i2c_clt1[idx] = i2c_client;
...
static const struct of_device_id es7210_dt_ids[] = {
#if ES7210_CHANNELS_MAX > 0
        { .compatible = "everest,es7210_2ch", .data = (void *)0 },
        { .compatible = "everest,es7210",     .data = (void *)0 },
        { .compatible = "ES7210_MicArray_0", .data = (void *)0 },//es7210_0
#endif

#if ES7210_CHANNELS_MAX > 4
        { .compatible = "ES7210_MicArray_1", .data = (void *)1 },//es7210_1
#endif

#if ES7210_CHANNELS_MAX > 8
        { .compatible = "ES7210_MicArray_2", .data = (void *)2 },//es7210_2
#endif

#if ES7210_CHANNELS_MAX > 12
        { .compatible = "ES7210_MicArray_3", .data = (void *)3 },//es7210_3
#endif
	{ }
};
